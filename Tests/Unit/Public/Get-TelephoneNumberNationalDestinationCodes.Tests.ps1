...
Describe "Public cmdlet - Get-TelephoneNumberNationalDestinationCode (ByISO/ByCode)" {
    Context "1 ByISO parameter set" {
        It "1.1 Should not throw for a valid ISO3 code" {
            { Get-TelephoneNumberNationalDestinationCode -ISO "USA" } | Should -Not -Throw
        }
        It "1.2 Should return NationalDestinationCode objects for USA" {
            $result = Get-TelephoneNumberNationalDestinationCode -ISO "USA"
    ...
    Context "2 ByCode parameter set" {
        It "2.1 Should not throw for a valid country code" {
            { Get-TelephoneNumberNationalDestinationCode -Code "+1" } | Should -Not -Throw
        }
        It "2.2 Should return results for country code +1" {
            $result = Get-TelephoneNumberNationalDestinationCode -Code "+1"
            $result | Should -Not -BeNullOrEmpty
        }
    }
}
